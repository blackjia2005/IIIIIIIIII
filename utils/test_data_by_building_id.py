...
os.makedirs("../lib/test_by_building_id", exist_ok=True)

# 遍历 "dataset_by_device_id" 文件夹中的每个 CSV 文件
for filename in os.listdir("../lib/dataset_by_building_id"):
    # 构造完整的文件路径
    filepath = os.path.join("../lib/dataset_by_building_id", filename)

    # 读取当前 CSV 文件的前 10 行数据
    df = pd.read_csv(filepath, nrows=100)

    # 构造新的文件路径
    new_filepath = os.path.join("../lib/test_by_building_id", filename)
